  ...
  #HDNG
  HDNG <- read.csv("HDNG v3/HDNG_long.txt", header=T, sep="\t", stringsAsFactors=F)
  ...
  long1$sex <- ifelse(substr(long1$suffix,4,4)=="1", "M",
                      ifelse(substr(long1$suffix,4,4)=="2","V", NA))
  ...
  #save long format
  write.table(long1, file="Input/HDNG v3/HEDC_long.txt", quote=FALSE, sep ="\t", col.names=TRUE, row.names = F)
  
  #bind long1 & hdng
  HDNG <- HDNG[!(HDNG$variable %in% long1$variable),]
  HDNGplus <- rbind(HDNG, long1)
  write.table(HDNGplus, file="HDNG v3/HDNG+.txt", quote=FALSE, sep ="\t", col.names=TRUE, row.names = F)
