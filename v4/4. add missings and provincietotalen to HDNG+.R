  ...
  #add CBSNR & ACODE
  acodes <- HDNG[!duplicated(HDNG[,c("CBSNR", "ACODE", "NAAM")]), c("CBSNR", "ACODE", "NAAM")]
  length(which(duplicated(acodes$NAAM))) #0
  HEDE1 <- merge(HEDE1, acodes, by.x="naam", by.y="NAAM", all=T)
  HEDE2 <- merge(HEDE2, acodes, by.x="naam", by.y="NAAM", all=T)
  HEDrest <- merge(HEDrest, acodes, by.x="naam", by.y="NAAM", all=T)
  ...
  #save long format
  write.table(missingHDNG, file="HDNG v4/missingHDNG.txt", quote=FALSE, sep ="\t", col.names=TRUE, row.names = F)
  
  #bind long1 & hdng
  HDNGfinal <- rbind(HDNG, missingHDNG, prov)
  HDNGfinal$Citation <- NULL
  write.table(HDNGfinal, file="HDNG v4/HDNG+prov+missing.txt", quote=FALSE, sep ="\t", col.names=TRUE, row.names = F)
